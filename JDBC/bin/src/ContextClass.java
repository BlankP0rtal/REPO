...
public class ContextClass{
	...
	public void loadProducts(ResultSet resSet) throws NullPointerException, SQLException{
		...
		while(resSet.next()){
			...
			product.setAvailableStock(resSet.getInt("availStock"));
			product.setTax(resSet.getDouble("tax"));
			product.setOfferAvailStat(resSet.getBoolean("OfferAvail_Stat"));
			product.setFirstDateOfPurchase(resSet.getTimestamp("first_date_of_purchase"));
	...
	public List<Product> getUnsoldProducts(Timestamp from, Timestamp to) throws SQLException{
		...
		*/ 
		products.clear();
		prepareStat = connection.getCon().prepareStatement("select product.* from product left join bill_detail on product.pid = bill_detail.pid AND bill_detail.purchasedate between ? AND ? where product.availstock > 0 AND product.first_Date_of_purchase < ? AND bill_detail.pid IS NULL;");
		prepareStat.setTimestamp(1,from);
		prepareStat.setTimestamp(2,to);
	...
	public List<Customer> getTopCustomers(Timestamp from,Timestamp to) throws SQLException{
		customers.clear();
		prepareStat = connection.getCon().prepareStatement("select customer.*,sum(bill.totalamount) as total from customer INNER JOIN bill ON bill.cid = customer.id where bill.purchasedate between ? AND ? AND bill.totalamount = bill.amountpaid Group by customer.id order by total DESC;");
		prepareStat.setTimestamp(1,from);
		prepareStat.setTimestamp(2,to);
		//resSet = 
		loadCustomers(prepareStat.executeQuery());
